#!/usr/bin/env python3
"""
Fine-tune bert-base-uncased on LAMA T-REx with an optional embedding loss.

For each example the input is a masked sentence (e.g. "Allan Peiper was born
in [MASK], Australia") and the target is the single answer token (e.g. "Alexandra").
Multi-token answers are skipped, matching the standard LAMA evaluation protocol.

Steps
-----
1. Zero-shot evaluation on test split  (no fine-tuning)
2. Fine-tune with λ = 0  (CE only, baseline)
3. Fine-tune with λ ∈ {0.1, 0.5, 1.0}  (CE + L_embed_l2)
   - Early stopping on val split
   - Final accuracy reported on test split

Splits: 80% train / 10% val (early stopping) / 10% test (final reporting)

Outputs
-------
lama_results.csv   — top-1 accuracy (val + test) and CE per run
"""
...
BATCH_SIZE       = 32
MAX_EPOCHS       = 20
EVAL_EVERY_STEPS = 200   # ~4-5 evals per epoch
PATIENCE_EVALS   = 6     # ≈ 1.2 epochs with no improvement
LR               = 2e-5
MAX_LEN     = 128
LAMBDAS          = [4.0, 2.0, 1.0, 0.5, 0.1, 0.0]
SEED             = 42
OUT_CSV          = "lama_results.csv"
...
def evaluate(model, dl):
    ...
    for batch in dl:
        ...
        B      = logits.shape[0]
        logits_at_mask = logits[torch.arange(B), mask_pos] # (B, V)
...
if os.path.exists(OUT_CSV):
    ...
    done_lambdas = set(
        existing[existing["epoch"] == "final"]["lambda"].astype(str).tolist()
    )
...
for lam in LAMBDAS:
    ...
    E     = model.bert.embeddings.word_embeddings.weight  # tied, stays in graph
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        for batch in tqdm(train_dl, desc=f"  λ={lam} epoch={epoch}", leave=False):
            ...
            el   = embed_loss_l2(logits_at_mask, answer_ids, E) if lam > 0 else torch.tensor(0.0)
            loss = ce + lam * el
