#!/usr/bin/env python3
"""
Warm-start schedule: lambda=1.0 for steps 0-4000, then lambda=0.0 for steps 4000-6000.
12 seeds. Appends to lama_results_cosine_multi.csv with lambda="warmstart".
"""
...
BATCH_SIZE       = 32
MAX_STEPS        = 5000
SWITCH_STEP      = 2500   # lambda=1.0 before this, lambda=0.0 after
EVAL_EVERY_STEPS = 200
LR               = 2e-5
MAX_LEN          = 128
SEEDS            = [42, 123, 456, 789, 1011, 2024, 3000, 4000, 5000, 6000, 7000, 8000]
OUT_CSV          = "lama_results_cosine_multi.csv"
LAM_LABEL        = "warmstart"   # stored in the lambda column
...
for seed in SEEDS:
    if (seed, LAM_LABEL) in done_pairs:
        print(f"\nSkipping seed={seed} (already done)")
        continue

    print(f"\n{'='*60}\nSEED = {seed}  [warmstart: lambda=1.0 -> 0.0 at step {SWITCH_STEP}]\n{'='*60}")
    ...
    while not done:
        model.train()
        for batch in tqdm(train_dl, desc=f"  seed={seed} step={global_step}", leave=False):
            ids = batch["input_ids"].to(DEVICE); am = batch["attention_mask"].to(DEVICE)
            mp  = batch["mask_pos"].to(DEVICE);  ans = batch["answer_id"].to(DEVICE)
            out = model(input_ids=ids, attention_mask=am)
            B   = out.logits.shape[0]
            ...
            lam_val = 1.0 if global_step < SWITCH_STEP else 0.0
            el      = embed_loss_cosine(lam_l, ans, E) if lam_val > 0 else torch.tensor(0.0)
            loss    = ce + lam_val * el
            ...
            if global_step % EVAL_EVERY_STEPS == 0:
                ...
                frac = avg_el / (avg_ce + avg_el) if (avg_ce + avg_el) > 0 else 0
                phase = f"lam=1.0" if global_step <= SWITCH_STEP else "lam=0.0"
                print(f"  seed={seed} step={global_step} [{phase}] "
                      f"val={val_acc:.4f} ce={avg_ce:.4f} el={avg_el:.4f}")
                results.append({"seed": seed, "lambda": LAM_LABEL, "step": global_step,
    ...
    print(f"  seed={seed} FINAL test acc={test_acc:.4f}  (best val step={best_step})")
    results.append({"seed": seed, "lambda": LAM_LABEL, "step": best_step, "epoch": "final",
                    "val_acc": best_acc, "val_ce": None,
                    "train_ce": None, "train_embed_loss": None, "embed_frac": None,
                    "test_acc": test_acc, "test_ce": test_ce})
    pd.DataFrame(results).to_csv(OUT_CSV, index=False)
    done_pairs.add((seed, LAM_LABEL))
